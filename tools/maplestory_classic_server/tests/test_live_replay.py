...
class LiveReplayTest(unittest.TestCase):
    ...
    def test_mesos_pickup_plan_uses_capture_pair_and_runtime_balance(
        self,
    ) -> None:
        captured_drop = FieldDropSpawn(
            spawn_mode=1,
        ...
        captured = _CapturedMesosPickupAdmission(
            drop_spawn=captured_drop,
            drop_refresh=replace(captured_drop, spawn_mode=0),
            controller_release=MobControllerChange(
                control_level=0,
            ...
            release_frame=1539,
            request_frame=1630,
            release_delay_seconds=0.3863522,
            admission_delay_seconds=1.5921627,
        )
        plan_state = SimpleNamespace(
            phase=GameplayPhase.ACTIVE,
            pending_item_pickups=0,
        ...
        with patch(
            "maple_server.live_replay._captured_mesos_pickup_admission",
            return_value=captured,
        ):
        ...
        report = plan.safe_dict()
        self.assertEqual(report["kind"], "mesos")
        self.assertEqual(
            (report["mesos_before"], report["mesos_delta"], report["mesos_after"]),
            (4567, 4, 4571),
        )
        self.assertEqual(report["drop_position"], {"x": 676, "y": -2695})
        self.assertEqual(report["animated_source_offset"], {"x": 0, "y": -1})
        self.assertEqual(
            report["prediction"]["response_source"],
            "reactive_item_pickup_policy",
        )
        self.assertEqual(
            report["prediction"]["server_opcodes"],
            [311, 311, 281, 41, 49, 312],
        )
        self.assertEqual(plan.drop_spawn.owner_value_1, 42)
        self.assertEqual(plan.drop_spawn.owner_value_2, 42)
        self.assertEqual(plan.drop_spawn.drop_kind, FieldDropSpawn.MESOS)
        self.assertNotEqual(plan.drop_spawn.drop_object_id, 1234)
        self.assertEqual(
            plan.drop_spawn.source_mob_object_id,
            plan.controller_release.object_id,
    ...
    def test_mesos_pickup_timeout_removes_the_injected_drop(self) -> None:
        def packet(opcode: int) -> SimpleNamespace:
            return SimpleNamespace(
                to_bytes=lambda: opcode.to_bytes(2, "little")
            )
        ...
        with (
            ...
            patch(
                "maple_server.live_replay.plan_mesos_pickup_live_replay",
                return_value=plan,
            ),
            patch(
                "maple_server.live_replay._get_runtime_status",
