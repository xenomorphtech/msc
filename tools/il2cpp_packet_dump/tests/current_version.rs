...
fn manifest_prefers_semantic_shapes_over_exact_opaque_pins() {
    ...
    let world_entry = shapes
        .iter()
        .find(|shape| shape.name == "world_entry_request")
        .unwrap();
    assert_eq!(world_entry.opcode, 8);
    assert_eq!(world_entry.length, Some(66));
    assert_eq!(world_entry.operations.len(), 7);
    assert!(!shapes.iter().any(|shape| {
        shape
            .name
            .starts_with("observed_client_to_server_opcode_8_")
    }));
    ...
    assert_eq!(shape.operations.len(), 53);
    ...
    assert!(
        [31, 32, 34, 35, 36, 38, 40, 42, 43, 45, 46]
            .into_iter()
            .all(|index| matches!(
                &shape.operations[index],
                ShapeOp::Read {
                    kind: ReadKind::I32,
                    ..
                }
    ...
    assert!([33, 37, 39, 44].into_iter().all(|index| matches!(
        &shape.operations[index],
        ShapeOp::Read {
            kind: ReadKind::U8,
            ..
    ...
    assert!(
        !shape
            .operations
            .iter()
            .any(|operation| matches!(operation, ShapeOp::Bytes { .. }))
    );
    assert!(matches!(
